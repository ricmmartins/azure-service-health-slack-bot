...
| WSL applies standard Linux permissions to files in its Linux root file system. On Windows drives mounted through DrvFS, Linux metadata is not enabled by default and `chmod` has limited effect without it. | [File Permissions for WSL](https://learn.microsoft.com/windows/wsl/file-permissions) | Official. The baby-steps path requires cloning under the WSL Linux home directory and blocks secret storage from `/mnt/<drive>/...`. |
