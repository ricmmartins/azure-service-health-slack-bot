...
| WSL applies standard Linux permissions to files in its Linux root file system. On Windows drives mounted through DrvFS, Linux metadata is not enabled by default and `chmod` has limited effect without it. The DrvFS automount root is configurable. | [File Permissions for WSL](https://learn.microsoft.com/windows/wsl/file-permissions), [Advanced settings configuration in WSL](https://learn.microsoft.com/windows/wsl/wsl-config#automount-settings) | Official. The baby-steps path requires cloning under the WSL Linux home directory and resolves the selected AZD environment directory's mount type, source, and options instead of assuming `/mnt/<drive>/...`. |
