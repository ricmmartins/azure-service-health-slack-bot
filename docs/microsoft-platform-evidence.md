...
| `azd env set` stores the expanded value in the selected local AZD environment file. Microsoft warns not to store secrets in AZD `.env` files and provides secret-reference alternatives. | [Manage AZD environment variables](https://learn.microsoft.com/azure/developer/azure-developer-cli/manage-environment-variables#secrets-and-sensitive-data-considerations) | Official. This project has a bootstrap limitation because its target vault is created by the same provision that consumes the token. The current implementation retains the value in the protected local environment for later provisioning. |
| AZD serializes cross-process environment writes with an operating-system lock on `.env.lock`. | [Azure Developer CLI concurrency model](https://github.com/Azure/azure-dev/blob/main/cli/azd/docs/concurrency-model.md#lock-acquisition-order) | Official implementation contract. The secret-failure cleanup holds this lock across its read, filtered rewrite, and atomic replacement. Operators must not run another AZD command for the environment concurrently with stage 4. |
| Every current infrastructure provision consumes `SLACK_BOT_TOKEN` and writes it as the value of a new versioned Key Vault secret. The Bicep parameter does not reject an empty value. | `infra/main.parameters.json`, `infra/main.bicep`, and `infra/modules/security.bicep` | Implementation-specific. Operators must keep the real token in the protected named AZD environment, update that environment before rotation, and provision the same explicit environment. Rotating only in Key Vault is unsafe because a later provision can overwrite the value. Removing local plaintext persistence requires an IaC design change before production. |
