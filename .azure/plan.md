...
| [AZD custom prompts: hooks for custom logic](https://learn.microsoft.com/azure/developer/azure-developer-cli/custom-prompts#option-3-hooks-for-custom-logic) | Parameters populated by `preprovision` declare defaults so AZD does not prompt before running the hook. The defaults are empty only for input discovery; a stable Bicep `fail()` gate prevents an incomplete Secure Webhook configuration from reaching resource modules. |
